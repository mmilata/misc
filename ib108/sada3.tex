...
\\
\textbf{\Large{Dodělat $P \notin T[x..n] \Rightarrow P \notin T[y..n] \forall y
\geq x$}}
...
V~rekurzivním zanoření jistě algoritmus \textsc{Ext\_KMP} neudělá více práce
než~původní, platí tedy pro něj následující rekurzivní vztah 
$$ T(n,0) = c, T(n,m) \leq T(n,m-1)+m+n$$
Tedy
\textsc{Ext\_KMP}$\in \O(m\cdot n+m^2)$.
\\
\textbf{\Large{Předělat na lineární}}
\hfill$\square$
