...
porovnání znaku s~\uv{?} vždy vrací \textbf{true}. Toto zřejmě zachovává i
vlastnosti
...
$$ P=u.\{*\}.v \in T[1..n] \Leftrightarrow T[x..y] = u \wedge
v \in T[y+1..n] \mid u,v \in \Sigma^*, 1 \leq x \leq y$$
...
\IF {$q = m$} \RETURN $i-m$ je platný posun \STATE $q \la \pi[q]$ \ENDIF
\ELSE
\IF {$\textsc{Ext\_KMP}(T[i..n],P[q+1..m])$ vrátilo platný posun}
...
\noindent
Každé rekurzivní volání~\textsc{KMP} se volá na~alespoň o~jeden znak kratší
vzorek. Rekurzivní zanoření tedy nemůže být hlubší než~$m$. Protože skončí
původní~\textsc{KMP}, musí nutně skončit i~\textsc{Ext\_KMP}.

\noindent
Složitost funkce \textsc{Ext\_Prefixova\_funkce} je stejná jako v~původním
algoritmu, tedy~$\Theta(m)$. Složitost původního~\textsc{KMP} je~$\Theta(n)$.
Pro~\textsc{Ext\_KMP} pak platí
$$ T(n,0) = c, T(n,m) \leq T(n,m-1)+m+n$$
V~rekurzivním zanoření jistě algoritmus neudělá více práce než~původní.\\ Tedy
\textsc{Ext\_KMP}$\in \O(m\cdot n+m^2)$.
\hfill$\square$
