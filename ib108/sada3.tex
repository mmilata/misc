...
Jednoduše lze pozorovat, že algoritmus zkoumá každý znak textu i každý znak
vzorku pouze jedenkrát (stejně jako původní \textsc{KMP}). Při~rekurzivním
zanoření algoritmus začíná s~prvním dosud neprozkoumaným znakem textu,
prefixovou funkci počítá pro~první znak vzorku, který ještě nebyl napočítán.
Složitost \textsc{Ext\_KMP} je tedy stejná jako složitost původního \textsc{KMP}
(\textsc{Ext\_KMP}$\in \O(n+m)$). Důkaz obdobný jako u~původního \textsc{KMP}.
\\Z čehož plyne i konečnost algoritmu.
\hfill$\square$
