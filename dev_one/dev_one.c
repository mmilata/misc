...
static ssize_t dev_one_read(struct file * filp, char __user * buf, size_t count,
		     loff_t *fpos)
{
	size_t todo = count;
...
loff_t dev_one_seek(struct file * filp, loff_t offset, int orig)
{
        return filp->f_pos = 0;
}
...
void __exit dev_one_cleanup(void)
{
	kfree(sea_of_ones);
	cdev_del(&cdev);
	unregister_chrdev_region(device, 1);
